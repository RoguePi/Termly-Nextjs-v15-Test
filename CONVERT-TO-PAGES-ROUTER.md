# Convert Current Branch to Pages Router

Follow these steps to create a Pages Router version from your current App Router branch:

## Step 1: Create New Branch from Current Branch
...
Update `components/TermlyConsent.tsx` - remove `'use client'` and change hooks:
...
## Step 4: Move CSS File

```bash
# Windows
move app\globals.css styles\globals.css

# Unix/Mac
mv app/globals.css styles/globals.css
```

## Step 5: Create pages/_app.tsx
...
## Step 6: Create pages/_document.tsx
...
## Step 7: Create pages/index.tsx

Copy content from `app/page.tsx` and adjust:
...
## Step 8: Create Policy Pages

```bash
mkdir pages\privacy-policy
mkdir pages\cookie-policy  
mkdir pages\terms-of-service
```

Create `pages/privacy-policy/index.tsx`, `pages/cookie-policy/index.tsx`, `pages/terms-of-service/index.tsx` with same content as app router versions.

## Step 9: Remove App Directory
...
```bash
git add .
git commit -m "Convert to Pages Router implementation"
...
# Create files (use the code above)
# - pages/_app.tsx
# - pages/_document.tsx  
# - pages/index.tsx
# - Update components/TermlyConsent.tsx
...
# Commit
git add .
git commit -m "Convert to Pages Router"
git push origin pages-router
```
